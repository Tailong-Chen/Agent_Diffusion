...
def evaluate(model_without_ddp, args, epoch, batch_size=64, log_writer=None):
    ...
    if log_writer is not None:
        ...
        if args.img_size == 256:
            ...
            # For other image sizes (e.g., 1024), skip FID/IS but keep images
            print(f"[Epoch {epoch}] No reference statistics for image size {args.img_size}")
            print(f"[Epoch {epoch}] Generated images saved to: {save_folder}")
            compute_metrics = False
        ...
        if compute_metrics:
            ...
            # Remove folder after computing metrics
            shutil.rmtree(save_folder)
        else:
            # Keep generated images for manual inspection
            print(f"[Epoch {epoch}] Images kept for inspection (no metrics computed)")
